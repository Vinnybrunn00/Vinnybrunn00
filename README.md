## Hello, I'm Vinícius Bruno!

I'm a **Flutter developer** and **IT professional**, driven by curiosity and a genuine desire to build things that are truly useful. I work on creating apps, APIs, and automations, and I also handle maintenance, support, and system deployment when needed. I enjoy turning ideas into something real—something that works, helps people, and helps me grow as a developer.

I'm currently focused on improving my skills by studying:

- Architecture and best practices (Clean, Modular, MVVM)
- State management (Provider)
- Building REST APIs with Python and Node.js
- Optimization and animations in Flutter
- Design patterns

I believe technology is about **solving problems**, not just writing code.

---

### 💡 Languages & Tools
[![My Skills](https://skillicons.dev/icons?i=flutter,dart,python,nodejs,cpp,typescript&theme=dark)](https://skillicons.dev)

---

<div align="center">

### 📊 Top Languages  
<img height="450" width="450" src="https://github-readme-stats-fast.vercel.app/api/top-langs?username=vinnybrunn00&layout=compact&theme=cobalt" alt="Top Languages"/>

</div>

---

<div align="center">

### 🔥 GitHub Activity & Performance  
<img height="550" width="550" src="https://github-readme-stats-fast.vercel.app/api?username=vinnybrunn00&show_icons=true&theme=cobalt&include_all_commits=true&count_private=true" alt="GitHub Stats"/>

### ⚔️ Streaks (consecutive contribution days)  
<img height="550" width="550" src="https://streak-stats.vercel.app/?user=vinnybrunn00&theme=cobalt" alt="GitHub Streak Stats"/>

</div>

---

## 📱 What I Develop

- Android/iOS apps with Flutter  
- REST APIs with Python or Node.js  
- Firebase integrations  
- Automations and intelligent scripts  
- Internal tools for companies  
- Technical support, backups, and maintenance  

---

## 🎯 Current Goals

- Publish apps on the Play Store  
- Build projects with solid architecture  
- Master animations and advanced UI in Flutter  
- Contribute to open source projects  
